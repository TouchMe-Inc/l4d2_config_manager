...
void ShowMenu(int iClient, ArrayList hActiveMenu)
{
    ...
    if (!ConfigManager_IsConfigLoaded()) {
        ...
        ConfigInfo info;
        if (!g_smConfigInfo.GetArray(szConfigName, info, sizeof(ConfigInfo))) {
            menu.SetTitle("%T", "MENU_MAIN_TITLE_WITH_UNKNOWN", iClient);
        } else {
            menu.SetTitle("%T", "MENU_MAIN_TITLE_WITH_CONFIG", iClient, info.name);
        }
    }
...
void ShowConfigMenu(int iClient, char[] szConfigName)
{
    Menu menu = CreateMenu(HandlerConfigMenu, MenuAction_Select|MenuAction_End|MenuAction_Cancel);

    ConfigInfo info;
    g_smConfigInfo.GetArray(szConfigName, info, sizeof(info));
    ...
    menu.AddItem(szConfigName, szBuffer);

    FormatEx(szBuffer, sizeof(szBuffer), "%T", "PRINT_DESCRIPTION", iClient);
    menu.AddItem(szConfigName, szBuffer, info.description != null && info.description.Length > 0 ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);

    menu.Display(iClient, -1);
}

public int HandlerConfigMenu(Menu menu, MenuAction hAction, int iClient, int iItem)
{
    switch (hAction)
    {
        case MenuAction_End: delete menu;
        ...
        case MenuAction_Cancel: {
            if (iItem == MenuEnd_Cancelled) {
                ShowMenu(iClient, g_hActiveMenu[iClient]);
            }
        }
...
Action HandlerVoteMatchEnd(NativeVote hVote, VoteAction tAction, int iParam1, int iParam2)
{
    switch (tAction)
    {
        case VoteAction_Display:
        {
            ...
            ConfigInfo info;
            if (!g_smConfigInfo.GetArray(g_szTargetConfig, info, sizeof(ConfigInfo))) {
                FormatEx(szVoteDetails, sizeof(szVoteDetails), "%T", "VOTE_MATCH_END", iParam1, g_szTargetConfig);
            } else {
